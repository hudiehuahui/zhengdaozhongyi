    ...
    <form id="pagerForm" data-toggle="ajaxsearch" action="<c:url value='/back/system/queryAllAdmin.do'/>" method="post">
        ...
        <div class="bjui-searchBar">
            ...
            <select name="role" data-toggle="selectpicker">
				...
				<option value="${ bxt:getEnumValue('AdminRoleStatusEnum','ADMIN')}" <c:if test="${roleId eq bxt:getEnumValue('AdminRoleStatusEnum','ADMIN')}">selected</c:if>>${bxt:getEnumName("AdminRoleStatusEnum","ADMIN")}</option>
			</select>&nbsp;
            <label>状态:</label>
    ...
    <table class="table table-bordered table-hover table-striped table-top" data-selected-multi="true">
        ...
        <c:forEach items="${po.results}" var="info" varStatus="s">
            <tr data-id="${info.ID}">
                ...
                <td>${info.relName}</td>
                <td>
	                <c:if test="${info.roleId eq bxt:getEnumValue('AdminRoleStatusEnum','ADMIN')}">${bxt:getEnumName("AdminRoleStatusEnum","ADMIN")}</c:if>
                </td>
                <td>
